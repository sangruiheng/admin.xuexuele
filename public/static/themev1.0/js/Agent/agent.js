...
layui.use(['element','jquery','layer','form','laypage'], function() {
    ...
    form.on('submit(formSearch)', function(data){
        if(data.field.province=="" && data.field.city=="" && data.field.county=="" && data.field.state=="" && data.field.identity=="" && data.field.keyword==""){
            layer.msg("搜索内容不能为空！");
        }else{
            getDataLists(1,data.field.province,data.field.city,data.field.county,data.field.state,data.field.identity,data.field.keyword);
        }
        return false;
    });
    ...
    function floatDataTpl(data){
        layui.each(data, function(index, item){
            ...
            if(item.area == "--地区--"){
                var site = item.province+" "+item.city;
            }else{
            ...
            innerData +='<td>'+item.superior_agent_name+'</td>';
            innerData +='<td>'+item.phone+'</td>';
            innerData +='<td>'+site+'</td>';
            innerData +='<td>'+item.proportion+'</td>';
            innerData +='<td>'+item.subordinate_count+'</td>';
            innerData +='<td>'+state+'</td>';
            setStateStr = '';
            innerData +='<td class="project-action">';
            innerData += '<a href="'+adminurl+'/agent/view/'+item.id+'" class="layui-btn layui-btn-small handle" data-title="fds"><i class="layui-icon"></i>查看</a> ';
    ...
    function getDataLists(page,province,city,county,state,identity,keyword){
        $.ajax({
            url: adminurl + "/agent/lists?page="+page + "&province="+ province +"&city="+ city +"&county="+ county +"&state="+ state +"&identity=" + identity +"&keyword=" + keyword,
            data:'',
            type:"get",
            dataType:"json",
            beforeSend: function () {
                    ...
                    if(res.count==0){
                        ...
                        laypage.render({
                            ...
                            ,jump: function(obj, first){
                                ...
                                if(!first){
                                    $.ajax({
                                        url: adminurl + "/agent/lists?page="+obj.curr + "&province="+ province +"&city="+ city +"&county="+ county +"&state="+ state +"&identity=" + identity +"&keyword=" + keyword,
                                        data:'',
                                        type:"get",
                                        dataType:"json",
                                        beforeSend: function (request) {
