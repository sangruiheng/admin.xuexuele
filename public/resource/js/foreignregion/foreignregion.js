layui.use(['element', 'jquery', 'layer', 'form', 'laypage', 'laydate','table'], function () {
    ...
    var active = {
        delete: function () {
            ...
            layer.confirm(auditMsg, {
                ...
                $.ajax({
                    url: adminurl + "/foreignregion/delete",
                    data: {
                        id: id,
                        status:state,
                        _token: _token,
    ...
    function getDataLists(page) {
        $.ajax({
            url: adminurl + "/foreignregion/lists",
            data: {
                page:page,
                paged:15,
                _token: _token
    ...
    function floatDataTpl(data, page) {
        layui.each(data, function (index, item) {
            ...
            innerData += '<td>';
                innerData += '<a  href= "' + adminurl + '/foreignregion/foreignregionview/' + item.id + '" class="layui-btn layui-btn-sm layui-btn handle" data-id="' + item.id + '" data-state="2"><i class="layui-icon"></i>修改</a>';
